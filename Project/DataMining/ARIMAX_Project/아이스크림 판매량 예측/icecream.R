...
cons_seasonal = diff(cons,lag=12)
income_seasonal = diff(income,lag=12)
price_seasonal = diff(price,lag=12)
temp_seasonal = diff(temp,lag=12)

a <- arima(cons_seasonal, order=c(1,0,0))
...
# CCF 확인
b = arima(income_seasonal,order=c(1,0,0),fixed=c(a$coef))
c = arima(price_seasonal,order=c(1,0,0),fixed=c(a$coef))
d = arima(temp_seasonal, order=c(1,0,0),fixed=c(a$coef))
...
su1.fit <- arimax(su1[,1],order=c(1,0,0),xtransf=su1[,2],transfer=list(c(0,0)))
su1.fit
...
model1 <- arima(diff(temp,differences = 1),order=c(1,0,0))
...
final <- su1.fit$coef[2] + su1.fit$coef[1] * diff(cons,lag=12)[13] + su1.fit$coef[3] * temp_fore$pred[1]
final_vec <- c(final)

for (i in seq(2,5)){
  final <- su1.fit$coef[2] + su1.fit$coef[1] * final + su1.fit$coef[3] * temp_fore$pred[i]
...
(series[26:30,"cons"] - (series[14:18,"cons"] + final_vec))/series[26:30,"cons"]*100
