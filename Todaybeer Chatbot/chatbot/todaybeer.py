...
model_intention.compile(loss="categorical_crossentropy", optimizer="adam", metrics=["accuracy"])
global graphs
...
class todaybeer():
    ...
    def engine(self, corpus, beer, preference, intention=False):
        ...
        self.tests = twitter_tag.morphs(corpus)
        ...
        self.return_intention = self.model_intention.predict_classes([vectorized_seq])
        self.return_classes = self.model.predict([vectorized_seq, np.array([beer]), self.preference])
        if self.return_intention == 0:
            return_classes = np.argmax(self.return_classes[0][0])
            if intention:
                print("정보판단")
            else:
                pass
