...
##2) 크기는 주어지지 않았지만, 허용한계는 주어졌으므로, 이를 역으로 이용하면 integrate(f=((1/gamma(2))*v^(n-2)*(1-v)),from=0, to = 0.5)
##가 0.95가 되어야 한다.
...
6
toler <- function(x,n = 48,j = 45,i = 4){
  return((gamma(n+1)/(gamma(j-i)*gamma(n-j+i+1)))*x^(j-i-1)*(1-x)^(n-j+i))
}
#의 적분을 취해준다.
#적분을 취하고, 적분 식 밖으로 빠져나간 상수항인 gamma항을 곱해주면


1 - integrate(toler,lower=0.75,upper=1)[[1]]
