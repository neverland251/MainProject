...
### (3) 이를 예측 분포 pdf에 대한 기댓값으로 표현하면
#E(a*L(a,b^2/n)) / E(L(a,b^2/n)) 이 된다.
### (4) w(a) = L(a,b^2/n) 로 다시 정의할 경우, 사후 분포가 아닌 새롭게 정의한 분포 분포 W(a) 하에서 기댓값 E(a)는 
### (1/n) * sum(a) 는 E(a)로 확률수렴하는 관계로 정의할 수 있다. 이것이 몬테카를로 알고리즘의 베이지안 추정해가 된다.
...
# P|C ~ h(p|c) = c*p^(c-1)
# C ~ gamma(1,a)
## 1) 손실함수로 MSE가 주어졌을 때, 이 계층적 베이지안 모형의 모수 P에 대한 베이지안 추정량을 구하시오
### 1) MSE하에서 베이지안 추정량은 평균이 되고, 따라서, 이는 E(p)와 같다.
### 2) E(p)를 구하기 위해 3단계 계층적 베이지안 모형은 두개의 모수(모수, 하이퍼모수)에 대한 적분을 실시한다. 
#### (1)우선, 하이퍼모수 c에 대하여 적분하면
# integrate f(y|p)*h(p|c)*u(c) dc = {a^2/[1-alog(p)]^2}*p^(y-1)*(1-p)^(n-y+1-1)

### 3) 계층적 베이지안 하에서, 기댓값 p는
# E(p) = {integrate(p * integrate f(y|p)*h(p|c)*u(c) dc)dp} / {integrate(integrate f(y|p)*h(p|c)*u(c) dc)dp}
#### (1) integrate( p * {a^2/[1-alog(p)]^2}*p^(y-1)*(1-p)^(n-y+1-1) dp) / integrate( {a^2/[1-alog(p)]^2}*p^(y-1)*(1-p)^(n-y+1-1) dp)
#### (2) 같은 dp 적분식 안이므로, {a^2/[1-alog(p)]^2}를 분모 분자에서 동시에 소거하면, 
#### integrate( p * p^(y-1)*(1-p)^(n-y+1-1) dp) / integrate( p^(y-1)*(1-p)^(n-y+1-1) dp)

### 4) 흥미롭게도, 이는 beta(y+1,n*-y+1)의 기댓값을 구하는 공식이 된다.
### 5) (1/n) * sum(p) 는 대수의 약법칙에 따라 E(p)로 확률 수렴하므로, p에 대한 몬테카를로 알고리즘을 통해 발생한 p값들의 평균은 베이지안 추정량이 된다.
